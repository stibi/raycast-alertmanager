...
import { getLastSilenceDuration, setLastSilenceDuration } from "./storage";
...
export function SilenceForm({ alert, onSilenced }: { alert: AlertWithInstance; onSilenced?: () => void }) {
  ...
  useEffect(() => {
    getLastSilenceDuration().then((d) => setLastDuration(String(d)));
  }, []);
  ...
  async function handleSubmit(values: { duration: string; createdBy: string; comment: string; matchers: string[] }) {
    ...
    try {
      ...
      await setLastSilenceDuration(durationHours);
  ...
  return (
    ...
    >
      ...
      <Form.TextField id="createdBy" title="Author" placeholder="your name or email" />
      <Form.TextArea id="comment" title="Comment" placeholder="Reason for silencing this alert..." />
    </Form>
  );
}
